...
CustomEvent.prototype.raise = function(sender, args) {
    var i, eventObject = this;
    for (i = 0; i < eventObject.fn.length; i++) {
        eventObject.fn[i](sender, args);
    }
};
...
CustomEvent.prototype.unsubscribe = function(fn) {
    var i, eventObject = this;
    for (i = 0; i < eventObject.fn.length; i++) {
        if (eventObject.fn[i] == fn) {
            eventObject.fn.splice(i, 1);
            break;
        }
    }
};
...
AudioPlayer.prototype.createPlaylist = function(audioFilePathArray) {
    ...
    if (audioFilePathArray && fileArrayLength > 0) {
        ...
        for (var i = 0; i < fileArrayLength; i++) {
            audioPlayerObject.defaultPlaylist.songs[i] = audioFilePathArray[i];
        }
    }
}
